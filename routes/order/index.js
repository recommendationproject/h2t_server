...
router.get('/byuser', async (req, res, next) => {
  ...
  router.get('/detailbyuser', async (req, res, next) => {    
    let rs = await dbs.execute(`SELECT od.id odid, od.order_id, p.id, od.product_id, od.amount, p.name, p.price, i.images, od.rating, od.comment FROM order_detail od, product p, images i WHERE i.product_id = p.id and od.product_id = p.id and od.order_id = ? group by i.product_id having min(i.id)`, [req.headers.id]);
    res.json(rs);
  });
